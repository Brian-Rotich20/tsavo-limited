    ...
    <div className="min-h-screen bg-white">
      ...
      <main className="max-w-6xl mx-auto px-6 py-12">
        ...
        <section className="mt-24 pt-12 border-t border-gray-100">
          <h2 className="text-2xl font-light text-gray-900 mb-12">More signage solutions</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            {products
              .filter(p => p.category === 'signage' && p.id !== product.id)
              .map((relatedProduct) => (
                <Link 
                  key={relatedProduct.id} 
                  href={`/signage/${relatedProduct.id}`}
                  className="group"
                >
